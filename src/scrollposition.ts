...
export class RememberScrollposition {
    ...
    if (!view ||!data) {
      console.error('restoreScrollposition was called with invalid parameters.')
      return;
    }
    ...
    const lastPosition = data.scrollpositions.find(p => p.path === view?.file?.path)?.scrollposition

    // TODO check how old the scroll position is and ignore it if needed
